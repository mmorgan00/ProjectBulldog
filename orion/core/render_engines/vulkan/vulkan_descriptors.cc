...
VkDescriptorPool DynamicDescriptorAllocator::get_pool(VkDevice device) {
...
VkDescriptorPool DynamicDescriptorAllocator::create_pool(
...
void DynamicDescriptorAllocator::init(VkDevice device, uint32_t maxSets,
...
void DynamicDescriptorAllocator::clear_pools(VkDevice device) {
...
void DynamicDescriptorAllocator::destroy_pools(VkDevice device) {
  for (auto p : readyPools) {
...
//> Descriptor Writer
/**
 * @brief Writes a Descriptor Buffer to the bound shader
 * @param binding The binding slot on the shader
 * @param buffer The buffer to be written to the shader
 * @param offset Offset into buffer to start at
...
void DescriptorWriter::write_buffer(int binding, VkBuffer buffer, size_t size,
  ...
  write.dstSet = VK_NULL_HANDLE;  // left empty for now until we need to write
                                  // it
...
/**
 * @brief Writes a Descriptor image to the bound shader
 * @param binding The binding slot on the shader
 * @param image The image to be written, optional
 * @param sampler The sampler to be written, optional
 * @param layout Image layout of bound image
 * @param type Descriptor Type
 */
