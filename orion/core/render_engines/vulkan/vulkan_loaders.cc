...
std::optional<std::shared_ptr<LoadedGLTF>> vkutil::loadGltfBinary(
    VulkanEngine* engine, std::string_view filePath) {
  OE_LOG(VULKAN_ENGINE, INFO, "Loading Gltf binary {}", filePath);
  ...
  fastgltf::GltfDataBuffer data = std::move(expectedData.get());
  ...
  if (type == fastgltf::GltfType::glTF) {
    ...
    OE_LOG(VULKAN_ENGINE, WARN, "Unsupported GLTF format");
    return {};
    // }
    ...
    if (load) {
      ...
      OE_LOG(VULKAN_ENGINE, ERROR,
             "Failed to load glTF: ", fastgltf::to_underlying(load.error()));
      return {};
    }
  } else {
    OE_LOG(VULKAN_ENGINE, ERROR, "Failed to determine glTF container");
    return {};
  }
  ...
  for (fastgltf::Mesh& mesh : gltf.meshes) {
    ...
    for (fastgltf::Primitive& p : mesh.primitives) {
      ...
      // load vertex positions
      {
        ...
        // primitive to have a POSITION, so it's perfectly valid to assert
        // that positionIt is never nullptr.
        auto positionIt = p.findAttribute("POSITION");
        fastgltf::Accessor& positionaccessor =
            gltf.accessors[positionIt->accessorIndex];
