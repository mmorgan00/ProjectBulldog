...
// TODO: Configuraiton driven, possibly in a constructor, rather than in an
// init() call
...
/**
 * @brief Creates an AllocatedImage and loads the provided data payload
 * @detail Calls original create_image, but also allocates a staging buffer and
 ...
 * Due to very tiny amount of pipelines currently used, creating all of the
 * pipelines up front is fine As the project grows, prioritizing some pipelines
 * and saving others for creating later may be justified
 */
 ...
 * @detail Background pipelines right now are all compute shader based images
 * rather than a cube map for now.
 */
void VulkanEngine::init_background_pipelines() {
  ...
  _mainDeletionQueue.push_function(
      [_device = _device, _gradientPipelineLayout = _gradientPipelineLayout,
       sky_pipeline = sky.pipeline, gradient_pipeline = gradient.pipeline]() {
        vkDestroyPipelineLayout(_device, _gradientPipelineLayout, nullptr);
        vkDestroyPipeline(_device, sky_pipeline, nullptr);
        vkDestroyPipeline(_device, gradient_pipeline, nullptr);
      });
}

/**
 * @brief Creates the mesh rendering pipeline.
 * @detail Right now all meshes use the same pipeline for rendering. As more
 * objects with different 'materials' are added, this will grow
 */
...
/**
 * @brief Initializes descriptor sets needed for camera transformation and
 * texture referencing
 */
 ...
 * Handles camera inputs and updates the transformations and then initiates
 * scene graph traversal of updates
 */
 ...
 * @detail Iterates through the draw context for everything to be drawn in this
 * frame.
 * TODO: Room for improvement as each material/pipeline is bound per call.
 * Pre-sorting and drawing per pipeline would be more efficient as we scale
...
void VulkanEngine::draw_background(VkCommandBuffer cmd) {
  ...
  vkCmdDispatch(cmd, std::ceil(_drawExtent.width / 16.0),
                ...
                1); // transition the draw image and the swapchain image into
                    // their correct
}
 ...
 * @detail Handles input, issues IMGUI draw calls, and issues renderer driver
 * calls
 */
...
/**
 * @brief Creates a vulkan swapchain for the provided screen size. Used when
 ...
 * @detail Does not issue draw calls itself, but prepares draw resources then
 * queues self for rendering later. Recurses to child nodes
 * @param topMatrix
 * @param ctx
 */
